...
file *addFile(struct folder *f1,string name,string auth,string cont,string tp){
    file *f2=new file;
    f2->filename=name;
    f2->author=auth;
    f2->lastfolder=f1;
...
int main(){
    ...
    string enc_auth,enc_content,enc_type;
    string v;
    ...
    while(true){
        ...
        cin>>v;
            ...
            //2. create file: creates a new file takes in author name and enables to write content
		        ...
		        (p->nextfile).push_back(addFile(p,nm,enc_auth,enc_content,enc_type));
                }
            ...
            //5. read command to read the contents of the file
		        ...
		        for(x=0;x<(p->nextfile).size();x++){
		            ...
		            if(p->nextfile[x]->filename==qt){
		                ...
		                cout<<setw(20)<<left<<"Content: "<<decryptString(p->nextfile[x]->content,keytoread)<<endl;
		                break;
		            }
		        }
		        ...
		        for(x=0;x<(p->nextfile).size();x++){
		            ...
		            if(p->nextfile[x]->filename==qt){
		                ...
		                cout<<setw(20)<<left<<"Author: "<<decryptString(p->nextfile[x]->author,keytodecrypt)<<endl;
		                cout<<setw(20)<<left<<"Type: "<<decryptString(p->nextfile[x]->type,keytodecrypt)<<endl;
		                break;
		            }
		        }
